...
3. **分包商指标**：迪诺、科海、庆余堂和尚肯的接起率、处理时长、满意度、质检、解决率和占线率。
...
| MCC 热线工单 | 第一个 Sheet | 处理时长、热线满意度、热线解决率 |
...
```text
热线15秒接起率分母 = 当前分包商全部MCC通话记录
热线15秒接起率分子 = 响铃时间小于等于15秒的记录
热线15秒接起率 = 分子 ÷ 分母
```

响铃时间为空或无法识别时仍进入分母，不进入分子。
...
分母不按工单号去重。时间无效的记录按 0 增加时长，但仍占一个分母。
...
| `计算结果`的分包商指标 | 四个分包商的接起率、时长、满意度、质检、解决率和占线率 |
| `计算逻辑` | 指标来源和简要公式 |
| 六份源表 | 勾选“包含源表”时写入 |
...
3. MSP 的五类在线渠道名称是否精确。
4. 分公司字段是否为空或名称不统一。
5. 四个分包商的姓名、别名和工号是否都能命中当前映射。
6. 视频`是否接通`是否为`是/否`。
7. 视频失败原因是否为小写`canceled`。
8. 时间字段是否存在结束时间早于开始时间。
9. 质检表第一个“扣分”列是否就是实际考核列。
10. 质检合格记录是否真正为空，避免数字 0 被判为不合格。
...
| 热线 15 秒接起率 | MCC 通话 | 响铃不超过 15 秒 | 分包商全部 MCC 通话 |
| 视频 15 秒接起率 | 视频服务 | 接通且振铃不超过 15 秒 | A+B+C+D 定制口径 |
| 平均在线处理时长 | MCC 热线、视频服务 | 热线时长加视频通话时长 | 热线行数加视频行数 |
